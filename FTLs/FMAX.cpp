...
FMAX::FMAX(Ssd *ssd, Block_manager_parent* bm, Migrator* migrator) :
		FtlParent(ssd, bm),
		...
		logical_dependencies()
{
	...
	if (GREED_SCALE > 0) {
		printf("Warning: the parameter GREED_SCALE must be set to 0 for FMAX. We set it to 0 here on your behalf.\n");
	}
	GREED_SCALE = 0;
...
void FMAX::read(Event *event)
{
	scheduler->schedule_event(event);
}

void FMAX::register_read_completion(Event const& event, enum status result) {
...
void FMAX::queue_up(Event* e, Address const& lock) {
...
void FMAX::write_in_log_block(Event* event) {
	int block_id = event->get_logical_address() / BLOCK_SIZE;
...
void FMAX::choose_existing_log_block(Event* event) {
	int block_id = event->get_logical_address() / BLOCK_SIZE;
...
void FMAX::release_events_there_was_no_space_for() {
...
void FMAX::unlock_block(Event const& event) {
...
void FMAX::register_erase_completion(Event & event) {
	if (gc_queue.empty()) {
...
void FMAX::register_write_completion(Event const& event, enum status result) {
...
void FMAX::consider_doing_garbage_collection(double time) {
...
void FMAX::garbage_collect(int block_id, log_block* log_block, double time) {
...
void FMAX::trim(Event *event)
{
	printf("We don't allow trims for FMAX for now.\n");
	assert(false);
}

void FMAX::register_trim_completion(Event & event) {
	page_mapping.register_trim_completion(event);
}

long FMAX::get_logical_address(uint physical_address) const {
	return page_mapping.get_logical_address(physical_address);
}

Address FMAX::get_physical_address(uint logical_address) const {
	return page_mapping.get_physical_address(logical_address);
}

void FMAX::set_replace_address(Event& event) const {
	Address const& ra = page_mapping.get_physical_address(event.get_logical_address());
	event.set_replace_address(ra);
}

void FMAX::set_read_address(Event& event) const {
